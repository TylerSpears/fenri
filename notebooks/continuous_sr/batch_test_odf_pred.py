...
# %%
tmp_inr_res_dir = tmp_res_dir / "INR"
tmp_inr_res_dir.mkdir(exist_ok=True)
...
with torch.no_grad():
    ...
    for batch_dict in test_dataloader:
        ...
        if len(subj_id) == 1:
            subj_id = subj_id[0]
        print(f"Starting subj {subj_id}", flush=True)
        ...
        y_mask = batch_dict["mask"].to(torch.bool).to(device)
        y_coords = batch_dict["extent_acpc"].to(device)
        y_affine_vox2mm = batch_dict["affine_vox2acpc"].to(device)
        if y_affine_vox2mm.shape[0] == 1:
            y_affine_vox2mm = y_affine_vox2mm[0]
        ...
        superres_pred = pred_fodf.cpu().numpy()
        ...
        nib.save(
            nib.Nifti1Image(odf_coeffs, affine=output_affine),
            tmp_inr_res_dir
            / f"{subj_id}_prediction_odf-coeff_inr_{p.target_vox_size}mm.nii.gz",
        )
...
# %%
tmp_trilin_res_dir = tmp_res_dir / "tri-linear"
tmp_trilin_res_dir.mkdir(exist_ok=True)
...
with torch.no_grad():
    ...
    for batch_dict in test_dataloader:
        ...
        super_coords = einops.rearrange(y_coords, "b coord x y z -> b (x y z) coord")
        pred_fodf = pitn.affine.sample_3d(
            x, super_coords, x_affine_vox2mm, mode="bilinear", align_corners=True
        )
        pred_fodf = einops.rearrange(
            pred_fodf,
            "b (x y z) c -> b c x y z",
            x=y_coords.shape[2],
            y=y_coords.shape[3],
        ...
        nib.save(
            nib.Nifti1Image(odf_coeffs, affine=output_affine),
            tmp_trilin_res_dir
            / f"{subj_id}_prediction_odf-coeff_tri-linear_{p.target_vox_size}mm.nii.gz",
        )
