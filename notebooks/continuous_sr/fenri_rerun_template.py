...
###############################################
p.experiment_name = "test_inr_bvec-resample_fixed-mlp_subj_split-02.1"
...
p.train_val_test_split_file = (
    Path("./data_splits") / "HCP_train-val-test_split_02.1.csv"
)
...
system_state_dict = torch.load(
    "/data/srv/outputs/pitn/results/tmp/2023-07-01T11_52_46/best_val_score_state_dict_epoch_26_step_17335.pt"
)
encoder_state_dict = system_state_dict["encoder"]
optim_encoder_state_dict = system_state_dict["optim_encoder"]
optim_decoder_state_dict = system_state_dict["optim_decoder"]
decoder_state_dict = system_state_dict["decoder"]
curr_epoch = int(system_state_dict["epoch"])
curr_step = int(system_state_dict["step"])
aim_run_hash = system_state_dict["aim_run_hash"]
p.aim_logger["run_hash"] = aim_run_hash
# "epoch": epoch,
# "step": step,
...
try:
    ...
    with torch.no_grad():
        encoder(torch.randn(1, in_channels, 20, 20, 20, requires_grad=False).to(device))
    ...
    optim_encoder.load_state_dict(optim_encoder_state_dict)
    ...
    optim_decoder.load_state_dict(optim_decoder_state_dict)
    ...
    curr_best_val_score = 0.00023901
